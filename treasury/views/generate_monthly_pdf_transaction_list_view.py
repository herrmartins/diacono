...
@login_required
def GenerateMonthlyPDFTransactionListView(request, year=datetime.now().year,
    ...
    if request.user.has_perm('treasury.view_transactionmodel'):
        ...
        last_day_last_month = calendar.monthrange(monthly_balance_date.year,
                                                monthly_balance_date.month)
        subtotal = []

        monthly_balance = MonthlyBalance.objects.get(month=reports_date)

        balance_for_calc = MonthlyBalance.objects.get(
            month=monthly_balance_date).balance
        last_month_balance = balance_for_calc
        finance_entries = TransactionModel.objects.filter(
            date__month=month, date__year=year).order_by(
            "date",)
        for fe in finance_entries:
            balance_for_calc += fe.amount
            subtotal.append(balance_for_calc)
            print("Balanço após", fe.amount, fe.date,
                fe.description, ":", balance_for_calc)

        context = {'finance_entries': finance_entries,
                'subtotal': subtotal, "total": balance_for_calc,
                "month": reports_date.month,
                "year": reports_date.year,
                "previous_balance": last_month_balance,
                "last_month_date": monthly_balance_date.replace(
                    day=last_day_last_month[1]),
                }
