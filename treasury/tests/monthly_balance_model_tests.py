...
class MonthlyBalanceModelTest(TestCase):
    ...
    def test_monthly_balance_creation(self):
        ...
        self.assertEqual(self.balance.__str__(
    ...
    def test_monthly_balance_reconstruction(self):
        ...
        MonthlyBalance.objects.get(
            month=self.four_months_ago).delete(is_testing=True)
        MonthlyBalance.objects.get(
            month=self.one_month_ago).delete(is_testing=True)
    ...
    def test_updating_subsequent_monthly_balances(self):
        ...
        balance_to_edit = MonthlyBalance.objects.get(
            month=self.five_months_ago)
        ...
        new_balance_to_edit = MonthlyBalance.objects.get(
            month=self.four_months_ago)
        ...
        self.assertAlmostEqual(last_balance.balance,
                               value_to_assert, places=2)
